...
import { CrearOrdenForm } from '../ordenes/CrearOrdenForm'
...
export const Ordenes = () => {
  ...
  const getStatusColor = (status) => {
    if (status === 'REPARANDO') return 'bg-orange-100 text-orange-600 border border-orange-600'
    if (status === 'COMPLETADO') return 'bg-green-100 text-green-600 border border-green-600'
    if (status === 'EN_ESPERA') return 'bg-yellow-100 text-yellow-600 border border-yellow-600'
    if (status === 'ENTREGADO') return 'bg-yellow-100 text-green-800 border border-green-800'
  ...
  return (
    <div className="p-8">
      ...
      {!loading && !error && (
          <div className="overflow-x-auto border border-gray-200 rounded-lg shadow-md mt-4" >
